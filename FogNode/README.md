...
LLMonFHIR defaults to the host `spezillmfog.local` with HTTP and no auth, matching this compose file. No changes in the app are required when using these defaults.


## Prerequisites

- Docker and Docker Compose installed on Linux or macOS.
- Network access between the device running the LLMonFHIR app and the machine hosting the Fog Node (Devices must be connected to the **same network and subnet** with IP-level reachability).
- mDNS advertisement and discovery allowed on the network (required to resolve `.local` hostnames between LLMonFHIR and the Fog Node).
...
- Build and run the LLMonFHIR app on a physical device (the iOS simulator is not supported).
...
- As long as the Fog Node is reachable on your local network and the selected model in the LLMonFHIR app (e.g., `llama3.1:8b`) is pulled on the fog node, you’re set.
